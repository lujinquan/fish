...
//2.有分享社区（是否相同）
//3.相同（正常流程）
//4.不同（是否可切换社区）=> 禁止切换（正常流程）
//5.可以切换（用分享社区id查询商品详情和社区信息） => 商品是否存在
//6.存在（弹出切换提示）=> 确认切换关闭 => 返回原社区（重新请求）
//7.不存在（如果原小区也有这个商品，直接进入原小的这个商品）=> 否则跳转首页
  ...
  onLoad: function(options) {
    ...
    if (!currentCommunity_id) {
      ...
      util.getCommunityInfo(community).then(function(res){
        console.log('step1') 
        paramHandle();
        that.get_goods_details(options.id, res, currentCommunity_id);
      }).catch((param)=>{
      ...
      console.log('step3')
      paramHandle();
      that.get_goods_details(options.id, currentCommunity, currentCommunity_id);
              ...
              if (res.data.open_danhead_model == 1) {
                ...
                if (c_id != community_id && buy_type!='integral') {
                  if (community_id) {
                    ...
                    that.setData({ changeCommunity: community }, () => {
  ...
  get_goods_details: function (id, communityInfo='', currentCommunity_id){
    ...
    app.util.request({
      ...
      success: function (res) {
        ...
        // 会员
        let { is_supply, is_open_vipcard_buy, modify_vipcard_name, is_vip_card_member, modify_vipcard_logo, is_member_level_buy } = res.data;
