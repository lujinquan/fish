...
Page({
    ...
    app.util.request({
      ...
      success: function (res) {
        ...
        if (res.data.code == 0) {
          ...
          app.util.message('无数据', '/lionfish_comshop/moduleA/pin/index', 'error');
        }
      }
    })
  },
  ...
  preOpenSku: function(){
    var that = this;
    var { order, options, goodsComunityId, groupInfo } = that.data;
    ...
    app.util.request({
      ...
      success: function (res) {
        if (res.data.code == 3) {
          ...
          var pages_all = getCurrentPages();
          let url = `/lionfish_comshop/pages/order/placeOrder?type=${buy_type}`;
