...
<table class="table table-hover supplyquery" style="min-width:850px;">
    <tbody>   
        ...
        <tr>
          <td><img src='<?php echo tomedia($row['thumb']);?>' style='width:30px;height:30px;padding1px;border:1px solid #ccc' /> <?php echo ($row['goodsname']); ?></td>
          <td style="width:80px;"><a href="javascript:;" class="choose_dan_link_goods" data-json='<?php echo json_encode($row);;?>'>选择</a></td>
        </tr>
        <?php } ?>
...
var query_url = "<?php echo U('goods/query_normal', array('type' => $type,'template' => 'mult','is_not_hexiao' => $is_not_hexiao,'unselect_goodsid' => $_GPC['unselect_goodsid']));?>";
var can_next = true;
