...
<div class="layui-fluid">
	<div class="layui-card">
		<div class="layui-card-header layui-elem-quote">当前位置：<span class="line-text">支付设置</span></div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						...
						<div class="layui-form-mid">
							...
							<?php  }else{ ?>
							<span class="label label-danger">未上传</span>
							<?php } ?>从商户平台上下载支付证书，解压并取得其中的<span class="bg-danger">apiclient_key.pem</span>用记事本打开并复制文件内容，填至此处</div>
					</div>
				</div>
				<div class="layui-form-item" style="display:none;">
					<label class="layui-input-block">ROOTCA证书</label>
					<div class="layui-input-block">
						<textarea style="height:100px;" name="parameter[wechat_rootca_pem]" class="form-control" rows="5" placeholder="<?php if( $data[wechat_rootca_pem]){ ?>为保证安全性，不显示证书内容。若要修改请直接输入<?php } ?>"></textarea>
						<div class="layui-form-mid">
							<?php if( $data[wechat_rootca_pem]){ ?>
							<span class="label label-primary">已上传</span>
							<?php  }else{ ?>
							<span class="label label-danger">未上传</span>
							<?php } ?>从商户平台上下载支付证书，解压并取得其中的<span class="bg-danger">rootca.pem</span>用记事本打开并复制文件内容，填至此处</div>
					</div>
				</div>
