	...
	<div class="layui-card">
		...
		<div class="layui-tab layui-tab-brief" >
		  <ul class="layui-tab-title">
				<li  <?php if(empty($type) || $type=='all'){ ?>class="layui-this"<?php } ?>><a href="<?php echo U('points/goods');?>">全部商品（<?php echo ($all_count); ?>）</a></li>
                <li  <?php if($type=='saleon'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('points/goods',array('type'=>'saleon'));?>">出售中（<?php echo ($onsale_count); ?>）</a></li>
                <li <?php if($type=='stock_notice'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('points/goods',array('type'=>'stock_notice'));?>">库存预警（<?php echo ($stock_notice_count); ?>）</a></li>
                <li <?php if($type=='getdown'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('points/goods',array('type'=>'getdown'));?>">已下架（<?php echo ($getdown_count); ?>）</a></li>
              
				<?php if($is_open_shenhe == 1){ ?>
				<li <?php if($type=='warehouse'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('points/goods',array('type'=>'warehouse'));?>">仓库（<?php echo ($warehouse_count); ?>）</a></li>
               <?php } ?>
			    <li <?php if($type=='recycle'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('points/goods',array('type'=>'recycle'));?>">回收站（<?php echo ($recycle_count); ?>）</a></li>
					...
					<div class="col-md-12">
						...
						<div class="page-table-header">
							...
							<span class="pull-right"> 
								<a href="<?php echo U('points/addgoods', array('ok' =>1));?>" class="layui-btn layui-btn-sm"><i class="fa fa-plus"></i> 添加商品</a>
							</span>
							<div class="btn-group">
						...
						<table class="table table-responsive" lay-even lay-skin="line" lay-size="lg">
							...
							<td>
								<a href="<?php echo U('points/editgoods', array('id' => $item['id'],'goodsfrom'=>$goodsfrom,'page'=>$page));?>">
						...
						<?php } ?>
								...
								<td  style="overflow:visible;position:relative">
									...
									<a  class='layui-btn layui-btn-xs' href="<?php echo U('points/editgoods', array('id' => $item['id'],'ok'=>1,'page'=>$page));?>"  >
...
var heads_page = 1;
    ...
    $('.model_heads').click(function(){
		...
		if(head_id_arr.length > 0)
		{
			...
			$.post("<?php echo U('goods/ajax_batchheads');?>",{'goodsids':goodsids,'head_id_arr': head_id_arr}, function (ret) {
	            if (ret.status == 1) {
	                $('#batchheads').hide();
	                layer.msg('分配成功');
   ...
   $("body").delegate(".cancle","click",function(){
	...
	$("body").delegate(".modal-group-head","click",function(){
        ...
        $.post("<?php echo U('goods/ajax_batchcates_headgroup');?>",{'goodsids':goodsids,'groupid': group_heads}, function (ret) {
            if (ret.status == 1) {
