...
class IndexController extends CommonController {
	...
	**/
		...
		if( !empty($head_id) && $head_id >0 )
		{
			...
			if(!empty($keyword)) {
				$sql_goods_ids = "select pg.goods_id from ".C('DB_PREFIX')."lionfish_community_head_goods as pg,"
	        	           ...
	        	           where pg.goods_id = g.id and g.goodsname like '%{$keyword}%' and pg.head_id = {$head_id} 
						    order by pg.id desc ";
			...
			if(!empty($keyword)) {
				$goods_ids_nolimit_arr = M('lionfish_comshop_goods')->field('id')->where("is_all_sale=1 and goodsname like '%{$keyword}%' ")->select();
			...
			if(!empty($keyword)) {
				...
				$goods_ids_nolimit_arr = M('lionfish_comshop_goods')->field('id')->where( " goodsname like '%{$keyword}%' and type='normal' " )->select();
				
			} else {
				if($gid == 0){
					
					$goods_ids_nohead_arr = M('lionfish_comshop_goods')->where(" type='normal' ")->field('id')->select();
	...
	public function load_spikebuy_goodslist_old()
	{
		$_GPC = I('request.');
