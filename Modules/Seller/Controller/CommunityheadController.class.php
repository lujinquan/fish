...
class CommunityheadController extends CommonController {
	...
	public function index(){
		...
		foreach( $list as $key => $val )
		{
			...
			$goods_count_arr = M()->query("select count(hg.id) as count from ".C('DB_PREFIX')."lionfish_community_head_goods as hg ,
								".C('DB_PREFIX')."lionfish_comshop_good_common as gc ,".C('DB_PREFIX')."lionfish_comshop_goods as g 
									where hg.goods_id = gc.goods_id and gc.goods_id = g.id and g.is_all_sale=0 and hg.head_id = ".$val['id'] );
			
			$val['goods_count'] = $goods_count_arr[0]['count'];
	...
	public function communityhead()
	{
		...
		foreach( $list as $key => $val )
		{
			...
			$goods_count_ar= M()->query("select count(hg.id) as count from ".C('DB_PREFIX')."lionfish_community_head_goods as hg ,".C('DB_PREFIX')."lionfish_comshop_good_common as gc ,".C('DB_PREFIX')."lionfish_comshop_goods as g 
								where hg.goods_id = gc.goods_id and gc.goods_id = g.id and g.is_all_sale=0  and hg.head_id =".$val['id'] );
			
			$val['goods_count']	= $goods_count_ar['count'];
