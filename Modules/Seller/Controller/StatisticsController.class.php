...
class StatisticsController extends CommonController{
		...
		switch( $type )
		{
			case 'normal':
				...
				if (defined('ROLE') && ROLE == 'agenter' ) 
				{
					...
					//total
					$today_pay_money_info = D('Seller/Order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $today_pay_where);
				...
				if (defined('ROLE') && ROLE == 'agenter' ) 
				{
					...
					//C('DB_PREFIX')
					$sq_s = "SELECT sum(total+shipping_fare-voucher_credit-fullreduction_money) as total FROM ".
							C('DB_PREFIX')."lionfish_comshop_order where order_status_id in (1,4,6,11,12,14) ";
				...
				if (defined('ROLE') && ROLE == 'agenter' ) 
				{
					...
					$wait_order_count = D('Seller/Order')->get_order_count(" and order_status_id =1 ");
				...
				if (defined('ROLE') && ROLE == 'agenter' ) {
					...
					$order_ids_list = M()->query("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".C('DB_PREFIX').
										"lionfish_comshop_order_goods as og , ".C('DB_PREFIX')."lionfish_comshop_order as o  where  og.order_id =o.order_id and og.supply_id = ".$supper_info['id']." and o.pay_time > {$begin_time} and o.order_status_id in (1,4,6,11,14) ");
					$order_ids_arr = array();
					...
					$seven_pay_where = " and pay_time > {$begin_time} and order_status_id in (1,4,6,11,14) ";
					...
					$seven_pay_money = empty($seven_pay_money_info['total']) ? 0:$seven_pay_money_info['total'];
				...
				if (defined('ROLE') && ROLE == 'agenter' ) {
					...
					$order_ids_list = M()->query("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".C('DB_PREFIX').
										"lionfish_comshop_order_goods as og , ".C('DB_PREFIX')."lionfish_comshop_order as o  where  og.order_id =o.order_id and og.supply_id = ".$supper_info['id']." and o.pay_time > {$begin_time} and o.order_status_id = 7 ");
					$order_ids_arr = array();
					...
					foreach($order_ids_list as $vv)
					{
						...
						{
							$order_ids_arr[$vv['order_id']] = $vv;
							$seven_refund_money += $vv['total']+$vv['shipping_fare']-$vv['voucher_credit']-$vv['fullreduction_money'];
						}
					}
					...
					$seven_refund_pay_where = "  and pay_time > {$begin_time} and order_status_id = 7 ";
				
					$seven_refund_money_info = D('Seller/Order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $seven_refund_pay_where);
					$seven_refund_money = empty($seven_refund_money_info['total']) ? 0:$seven_refund_money_info['total'];
		...
		switch( $type )
		{
			case 'normal':
				...
				if (!defined('ROLE') && ROLE != 'agenter' ) 
				{
					...
					$sum_info = D('Seller/Order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $where);
	...
	public function load_echat_month_head_sales()
	{
		...
		$sql = " select sum(total+shipping_fare-voucher_credit-fullreduction_money) as total,head_id from ".C('DB_PREFIX').
				"lionfish_comshop_order where  date_added >= {$begin_time} and date_added <={$end_time} and order_status_id in(1,4,6,11,14) 
			...
			case 'normal':
				...
				for($i =7; $i>=1; $i--)
				{
					...
					if (!defined('ROLE') || ROLE != 'agenter' ) {
						...
						$sum_info = D('Seller/Order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $where);
						$refund_sum_info = D('Seller/Order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $where_refund);
